...
| **2S balance-port low-voltage alarm/buzzer** | 1 | $5 | Audible over-discharge backstop now that the bot runs untethered off the LiPo. Deferred from Phase 1 (the LiPo's protection board covered over-discharge there). Remove between sessions. | https://hobbyking.com/en_us/cell-checker-with-low-voltage-alarm-2s-8s.html | **buy** |
...
> **Already on the bot (from Phases 1–2), used again here:** Raspberry Pi 5, Pico 2W, 2× TB6612FNG, 4× 310 motors+encoders, 2S LiPo (+ its SM-2P pigtail and WAGO splices), bulk + ceramic caps, screw-terminal carrier. No re-purchase.
>
> **New here, deferred out of Phase 1:** the **#2815 master switch**, the **coordinated fuse set (10 A / 7.5 A / 5 A)**, the **NC e-stop**, and the **2S low-voltage-alarm buzzer**. Phase 1's single-branch bench build on a self-protected LiPo, wheels-in-air, didn't need them; this two-branch, floor-driving phase does. The Phase 1/2 SM-2P → WAGO path is rebuilt into the full tree (SM-2P → main fuse → #2815 switch → motor fuse → e-stop → WAGO → VM).
>
...
> ⚠️ **Connector upgrade before you wire this phase.** The LiPo's factory **SM-2P** output is only **~3 A/contact** — fine for Phase 1's motor branch, but this phase hangs the Pi-buck (and later the servo UBEC) off the *same* battery output, pushing the pack-side draw to **~5.4 A continuous / ~10 A worst case**. Fit an **XT30/XT60** at the battery, or terminate the SM-2P immediately into a **fused distribution block** and pull every branch from that block — don't run whole-robot current through the 3 A SM contacts. (See `_engineering/power_budget.md`.)
