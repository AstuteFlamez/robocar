...
- **Phase 1 complete:** all four wheels spin under Pico control through the two TB6612FNG drivers, off the simplified bench power path (LiPo **ferruled leads** → **WAGO** splits → both drivers' VM/GND, polarity metered by hand), and the bot drives open-loop from the Pico REPL. (The full protection tree gets built *this* phase — see below.)
...
> Already on the bot from Phases 1–2 (no re-purchase): chassis, motors, encoders, TB6612 ×2, LiPo (on its ferruled leads + the WAGO splices), bulk + ceramic caps, screw-terminal carrier, Pi 5, Pico 2W. **New this phase** is the Pi branch *and* the full protection tree (main/branch fuses, #2815 switch, NC e-stop) that Phases 1–2 deliberately deferred — a single-branch bench build on a self-protected battery didn't need it; a two-branch, floor-driving robot does. Full master list: [`_engineering/products_to_buy.md`](../_engineering/products_to_buy.md).

---

## Where we are now — the cumulative system in prose

After Phases 1 and 2, the **Pico 2W** is a competent little motor brain. It runs two **TB6612FNG** dual H-bridge drivers (one motor per channel, four channels for four wheels — exactly what Mecanum requires). It throws **20 kHz PWM** at them — above hearing, so the motors don't whine — with **IN1/IN2 setting direction and PWM setting speed** in TB6612 standard mode. Both drivers' **STBY** pins are tied high to GP14, because a TB6612 with STBY low is a brick: its outputs stay disabled and the motors do nothing. The drivers' **VM** comes from the 7.4 V LiPo motor rail — so far just the bench path (ferruled leads → WAGO splits), which *this* phase upgrades into the fully-protected tree (main fuse → master switch → motor-branch fuse → NC e-stop); their **VCC logic rail and all four encoder VCCs** come from the Pico's **3V3(OUT)** pin, whose ~300 mA budget the whole logic load sits comfortably under. Each motor's **Hall encoder** feeds an A/B quadrature pair into the Pico's **PIO state machines** (one per wheel, four of twelve used), and a 50 Hz PID loop turns "0.3 m/s commanded" into "0.3 m/s on the floor" regardless of friction or battery sag.
...
- [ ] **Build the motor protection tree (battery leads out of the WAGOs).** Insert the chain ahead of the WAGO: LiPo red (+) ferrule → 10 A main fuse → #2815 master switch → 7.5 A motor fuse → NC e-stop → WAGO (+) → both VM. Slow-blow blades on main + motor. Beep-test every junction and confirm no V+↔GND short before powering. Meter the leads (red = +), seat them, then confirm **VM = 7.4 V** at both boards with the switch on and e-stop released — and that pressing the e-stop drops VM to 0 V while the Pico/Pi stay alive. (The #2815 now protects against a reversed lead, but verify polarity anyway.)
