...
- **Phase 1 complete:** all four wheels spin under Pico control through the two TB6612FNG drivers, off the simplified bench power path (LiPo **SM-2P** → female pigtail → **WAGO** splits → both drivers' VM/GND), and the bot drives open-loop from the Pico REPL. (The full protection tree gets built *this* phase — see below.)
...
| Pololu **#2815** reverse-polarity master switch | Now that there's a second branch, this becomes the system master ON/OFF **+** reverse-polarity protection — a P-FET high-side switch with no diode drop. |
| **Inline ATC/ATO fuse holders + blades: 10 A main, 7.5 A motor, 5 A Pi** | The full coordinated-fusing set finally earns its keep: main at the battery, smaller branch fuses downstream (selectivity). Slow-blow on main + motor branch to ride motor inrush. |
| **22 mm latching mushroom e-stop (1× NC)** | First floor-driving phase → a fail-safe physical motor kill (motor branch only); NC so a broken wire also stops the wheels. |
...
> Already on the bot from Phases 1–2 (no re-purchase): chassis, motors, encoders, TB6612 ×2, **WAGO splices**, bulk + ceramic caps, screw-terminal carrier, Pi 5, Pico 2W. **New this phase** is the Pi branch, the full protection tree (main/branch fuses, #2815 switch, NC e-stop) that Phases 1–2 deliberately deferred, *and* a **battery swap**: the kit's SM-2P LiPo (fine for the Ph1–2 motors-only ~2.6 A) gives way to the higher-current **URGENEX pack on a Deans T-plug**, because the full system now draws ~5.4 A cruise / ~10 A worst — over the SM-2P's ~3 A. A single-branch bench build on a self-protected battery didn't need the tree; a two-branch, floor-driving robot does. Full master list: [`_engineering/products_to_buy.md`](../_engineering/products_to_buy.md).

---

## Where we are now — the cumulative system in prose

After Phases 1 and 2, the **Pico 2W** is a competent little motor brain. It runs two **TB6612FNG** dual H-bridge drivers (one motor per channel, four channels for four wheels — exactly what Mecanum requires). It throws **20 kHz PWM** at them — above hearing, so the motors don't whine — with **IN1/IN2 setting direction and PWM setting speed** in TB6612 standard mode. Both drivers' **STBY** pins are tied high to GP14, because a TB6612 with STBY low is a brick: its outputs stay disabled and the motors do nothing. The drivers' **VM** comes from the 7.4 V LiPo motor rail — so far just the Ph1–2 bench path (kit LiPo's SM-2P pigtail → WAGO splits), which *this* phase upgrades into the fully-protected tree **fed by the new URGENEX pack on its Deans T-plug** (T-plug pigtail → main fuse → master switch → motor-branch fuse → NC e-stop); their **VCC logic rail and all four encoder VCCs** come from the Pico's **3V3(OUT)** pin, whose ~300 mA budget the whole logic load sits comfortably under. Each motor's **Hall encoder** feeds an A/B quadrature pair into the Pico's **PIO state machines** (one per wheel, four of twelve used), and a 50 Hz PID loop turns "0.3 m/s commanded" into "0.3 m/s on the floor" regardless of friction or battery sag.
...
The fix is the **D24V50F5: 6–38 V in (7.4 V is in range, and it still holds 5 V even when the pack sags to 6.0 V empty), 5 V / 5 A out, ~90 % efficient,** with built-in reverse-voltage protection and soft-start. At full Pi load it pulls **~3.5 A from the 7.4 V pack** (23.5 W ÷ 0.9 ÷ 7.4 V — converters trade voltage for current, they aren't free). Setting **`usb_max_current_enable=1`** in `/boot/firmware/config.txt` tells the Pi "the rail really can supply the current, stop capping the ports." Adding the Pi as a second branch is also what makes the **coordinated fuse set** worth installing now: a **10 A main fuse** at the battery, a **7.5 A motor-branch fuse**, and a **5 A Pi-branch fuse** — each branch smaller than the main, so a Pi/buck fault opens *that* branch without touching the motors or the main. That's **selectivity**, and it does nothing until there's more than one branch to isolate — which is exactly why Phase 1 skipped it and Phase 3 builds it.
...
- [ ] **Swap in the URGENEX battery + build the motor protection tree (battery unplugged at the T-plug).** Fit the **Deans T-plug female pigtail** to the new pack and insert the chain between the pigtail and the WAGO: T-plug (+) → 10 A main fuse → #2815 master switch → 7.5 A motor fuse → NC e-stop → WAGO (+) → both VM. Slow-blow blades on main + motor. Beep-test every junction and confirm no V+↔GND short before plugging in. Then plug in the T-plug and confirm **VM = 7.4 V** at both boards with the switch on and e-stop released — and that pressing the e-stop drops VM to 0 V while the Pico/Pi stay alive.
