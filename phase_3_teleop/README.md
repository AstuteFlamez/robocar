...
- [ ] **Build the motor protection tree (battery unplugged at the SM-2P).** Insert the chain between the pigtail and the WAGO: SM-2P (+) → 10 A main fuse → #2815 master switch → 7.5 A motor fuse → NC e-stop → WAGO (+) → both VM. Slow-blow blades on main + motor. Beep-test every junction and confirm no V+↔GND short before plugging in. Then plug in the SM-2P and confirm **VM = 7.4 V** at both boards with the switch on and e-stop released — and that pressing the e-stop drops VM to 0 V while the Pico/Pi stay alive.
