...
| **0.1 µF (100 nF) ceramic cap** (12-pack) | ≤4 | — | Brush-noise suppression — but **optional / add-only-if-noisy.** The 310 motors are **connectorized** (factory JST-PH 6-pin, no bare tabs) and **may already carry a factory cap** on the rear board, so build without these first. If the USB link drops or (Phase 2) encoder counts jitter, land one **across each channel's AO1↔AO2 / BO1↔BO2 at the driver** (no polarity, no motor surgery). | (have) | already have |
