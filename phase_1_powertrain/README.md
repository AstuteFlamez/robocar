...
**All four "310" Mecanum drive motors spin forward and reverse under Raspberry Pi Pico 2W control, commanded one wheel at a time from a serial terminal on your laptop.** By the end you will have verified each wheel's direction by hand, scoped the 20 kHz PWM that drives them, and stood up the *motor side* of the robot's power tree — battery, fuses, reverse-polarity/master switch, e-stop — built to real current numbers.
...
**Medium · one weekend.** The soldering (motor noise caps, power-tree terminations) and the careful, fused power bring-up are what take the time — the code is short. Do *not* rush the power section; that's the part that bites.
...
| **2S LiPo 7.4 V 2200 mAh 10C** (already have) | The one and only energy source for the whole robot. |
| **XT60 connector pair** | Keyed, current-rated main battery connector. |
| **Pololu #2815 MOSFET slide switch** | Master disconnect **and** reverse-polarity protection in one part. |
| **Inline ATC/ATO fuse holders + 10 A / 7.5 A blades** | Coordinated fusing: 10 A main (at the battery), 7.5 A on the motor branch. |
| **22 mm latching mushroom e-stop (1× NC)** | Instant physical motor kill; normally-closed so a broken wire also trips it. |
...
The *muscle* power is completely separate. A **2S LiPo** feeds, in order: a **10 A main fuse** right at the battery positive, then a **MOSFET slide switch** that is simultaneously the master ON/OFF and reverse-polarity protection, then a **7.5 A motor-branch fuse**, then a **normally-closed e-stop** in series, and finally into both TB6612 **VM** (motor-voltage) pins at 7.4 V. A **470–1000 µF bulk capacitor** sits across VM to swallow current surges, and a **0.1 µF ceramic** sits across each motor's terminals to quiet brush noise. The four motors connect through their **factory JST-PH 6-pin connectors** — the motor leads are live, the encoder conductors are present but parked (unread) until Phase 2.
...
### Fuses protect *wires*, not batteries

A common misconception: the fuse is there to protect the battery. It isn't — the 22 A-capable LiPo is the *strongest* thing in the chain. The fuse protects the **wires and connectors** downstream: if a wire shorts, the LiPo will happily push tens of amps through it until the insulation melts and something catches fire. A fuse is a deliberately weak link that opens *first*, before the wire overheats. We size it just above the worst-case current the wire should ever carry (motor branch worst case ≈ 5.6 A all-stall → 7.5 A fuse) and below the wire's safe ampacity (18 AWG handles ~10 A in short runs).

We also use **coordinated fusing** (a.k.a. *selectivity*): a **10 A main fuse** right at the battery, with *smaller* branch fuses downstream (7.5 A motor branch this phase; a 5 A Pi branch arrives in Phase 3). Because each branch fuse is smaller than the main, a fault on one branch blows *that branch's* fuse first — the main survives and the rest of the robot stays alive. If you made every fuse the same value you'd lose selectivity and a single motor short could take down the whole machine.

### Reverse-polarity protection: P-FET vs diode

Plugging a LiPo in backwards is the most common *fatal* mistake in this hobby — it instantly destroys drivers and regulators. The naive fix is a **series diode**: it only conducts one way, so a backwards battery does nothing. But a diode drops ~0.7 V *all the time*, even when correct, and at several amps that's several watts burned as heat for no benefit.

The better fix, and what we use, is a **P-channel MOSFET high-side switch** (the Pololu #2815). A P-FET wired as an ideal-diode/reverse-protector has milliohms of resistance when conducting the right way — almost no voltage drop, almost no heat — yet blocks a reversed battery completely. Bonus: the #2815 is *also* the master ON/OFF switch, so one part does two safety jobs. The "P-FET beats a diode" reasoning is a classic power-electronics lesson worth being able to explain.

### Fail-safe e-stop (normally-closed)

A moving robot needs an instant physical kill that doesn't depend on software. We use a **latching mushroom-button e-stop** wired **normally-closed (NC)** *in series in the motor branch only*. NC means the contacts are closed (current flows) in normal operation; pressing the mushroom *opens* them and cuts motor power. Two beautiful properties: (1) it kills only the wheels — the Pi and Pico stay powered so they can log *why* you hit it; and (2) because it's normally-closed, a **broken wire or loose connector also opens the circuit** and stops the motors. The *safe* state (motors off) is the *default* state. That's the essence of fail-safe design.
...
- [ ] **Dry continuity ritual (battery disconnected, switch OFF, e-stop pressed/IN).** Multimeter on beep. Confirm a continuous ground from the star point to: both TB6612 GND pins, the Pico GND, and the LiPo− lead. Confirm **no** beep between any V+ (VM, 3.3 V) and any ground — that would be a short.
...
- [ ] **Build and arm the power tree.** With the switch OFF, assemble LiPo+ → 10 A main fuse → #2815 switch → 7.5 A motor-branch fuse → NC e-stop → both VM. Check each fuse's continuity. Connect the battery. **Release the e-stop, switch ON.** Measure **VM = 7.4 V** (8.4 V if freshly charged) on *both* TB6612 VM pins. Anything wildly off → switch OFF immediately and recheck.
...
- [ ] **E-stop test.** With wheels spinning, **press the mushroom.** All four must coast to a stop immediately while the Pico stays alive (its LED stays on). Release to re-arm.
...
- **Battery on before logic.** Wrong order. Bring up logic first (Pico on laptop USB), *then* arm the motor branch (release e-stop, switch on). Logic must settle before motor power so the drivers don't glitch their outputs on a cold start.
...
**2. Confirm the motor rail (multimeter, DC volts).** With the power tree armed, measure across each TB6612 **VM ↔ GND**. **Expected: 7.4 V** nominal (up to ~8.4 V freshly charged, sagging toward 6.0 V as the pack empties). If you see ~5 V or 0 V, you mis-fused, mis-switched, or left the e-stop pressed.
...
**What you learned.** You now understand, concretely, how a microcontroller turns a number into motion: an H-bridge reverses current direction, IN1/IN2 pick that direction while a 20 kHz PWM sets average voltage (and stays above hearing), and a STBY pin gates the whole driver. You also built the *muscle half* of a real power tree to actual current numbers — coordinated fuses that protect wires (not the battery) with selectivity, a P-FET that gives reverse-polarity protection without a diode's voltage drop, a fail-safe normally-closed e-stop, and a single-point star ground that keeps amps of motor return current out of your logic reference. And you learned the discipline that runs through the whole project: scope the PWM, measure the rail, and never trust a number you haven't seen on an instrument.

**Résumé bullet (copy-ready).**
> Designed and bench-validated the motor-control front end of a 4-wheel Mecanum robot: drove four independent brushed-DC motors via dual TB6612FNG H-bridges at 20 kHz PWM from an RP2350 (Pico 2W) co-processor, and built a fused, reverse-polarity-protected single-LiPo power tree with a fail-safe e-stop and single-point star ground, sized to the spec-derived ~5.6 A all-stall load (4 × 1.4 A stall).
