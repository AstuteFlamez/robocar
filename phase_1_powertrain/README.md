...
| **0.1 µF ceramic caps** (BOJACK assortment kit) | Brush-noise suppression. Inspect each motor's rear PCB for a factory cap; **if none is visible, fit one across each channel's driver outputs** — no evidence the 310s include one, so default to fitting. |
...
- [ ] **Build and arm the power path (battery still unplugged).** Assemble it dry: SM-2P female pigtail → red (+) into WAGO #1 → both TB6612 VM; black (−) into WAGO #2 → both TB6612 GND. Land every power wire in its screw terminal and tug-test it. Double-check polarity with the meter (red = +) *before* plugging in — and confirm each driver's VM↔GND reads the right way round (SM-2P keying only guards the battery mate, not the hand-wired WAGO/screw terminals). Now **plug in the SM-2P.** Measure **VM = 7.4 V** (up to ~8.4 V if freshly charged) on *both* TB6612 VM pins. Anything wildly off → **unplug the SM-2P immediately** and recheck.
...
- **Skipping the bulk cap.** It usually "works" on the bench, then later sags the rail on stall. Fit the **470–1000 µF across VM** on each driver *now*. (For the **0.1 µF motor cap**: inspect each motor's rear PCB; there's no evidence the 310s include one, so **default to fitting** a 0.1 µF across each channel's driver outputs to keep brush noise off the USB link and the Phase-2 encoder counts.)
