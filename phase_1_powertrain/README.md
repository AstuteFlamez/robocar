...
**All four "310" Mecanum drive motors spin forward and reverse under Raspberry Pi Pico 2W control, commanded one wheel at a time from a serial terminal on your laptop, with the wheels in the air on a stand.** By the end you will have verified each wheel's direction by hand, scoped the 20 kHz PWM that drives them, and stood up the *motor side* of the robot's power path — from the LiPo's ferruled flying leads, split through lever-nut connectors, into both driver boards at real current numbers. (The full protection chain — coordinated fuses, a reverse-polarity/master switch, a fail-safe e-stop — is explained here but *built later*, once the robot leaves the stand and drives on the floor. See "Concepts," below, for why each is deferred.)
...
| **2S LiPo 7.4 V 2200 mAh 10C, w/ built-in protection board** (already have) | The one and only energy source for the whole robot. Its protection board already guards against overcharge, over-discharge, overcurrent, and short circuit. Its output is **two flying leads — red (+) and black (−) — crimped into ferrules** (no connector). ⚠️ No connector means **no keying**: meter polarity (red = +) before every power-up. Charge with the included 8.4 V balance charger (confirm your pack's charge connector). |
| **2× WAGO 221-413 lever-nut splices (3-conductor)** | The no-solder way to split one battery rail into two: one WAGO splits the **+** (LiPo + → both drivers' VM), the other splits the **−** (LiPo − → both drivers' GND). The battery's **ferruled leads drop straight in** (lift lever, insert, close). Rated ~20 A — far above the ~5.6 A all-stall load. |
...
| **12× 0.1 µF ceramic caps** | Suppress brush noise right at each motor body. |
...
The *muscle* power is completely separate, and for this bench phase it's deliberately simple. The **2S LiPo** has a **built-in protection board** (overcharge, over-discharge, overcurrent, and short-circuit cut-offs) and its output is **two flying leads — red (+), black (−) — terminated in crimped ferrules** (no connector). Each lead drops directly into a **WAGO 221-413 lever-nut connector** — one WAGO fans the **+** out to both TB6612 **VM** (motor-voltage) pins, the other fans the **−** out to both boards' **GND**. Both drivers sit in **parallel** on the same 7.4 V rail. Each board receives that power at **soldered 2.54 mm screw terminals** on its power edge, so wires land by screw rather than by soldering each one. A **470–1000 µF bulk capacitor** sits across VM to swallow current surges, and a **0.1 µF ceramic** sits across each motor's terminals to quiet brush noise. The four motors connect through their **factory JST-PH 6-pin connectors** — the motor leads are live, the encoder conductors are present but parked (unread) until Phase 2.

⚠️ **Because the battery leads are bare ferrules, nothing keys the polarity** — it is physically possible to land + and − in the wrong WAGOs, which would destroy both drivers instantly. So the discipline this phase is: **mark each WAGO (+/−), and meter the leads (red = +) before every single power-up.** This is the one habit that replaces the safety a keyed connector would have given you.

There is **no fuse, master switch, or e-stop in the bench path yet** — and that's a deliberate choice, not an oversight. The LiPo's own protection board already opens on a short or overcurrent, and with the wheels in the air on a stand you have three instant kills already: pull the **+** lead out of its WAGO (lift the orange lever), type `stop` in the serial loop, or pull STBY (GP14) low in firmware. The external coordinated fuses, reverse-polarity P-FET, and physical fail-safe e-stop earn their place once a second branch (the Pi, in Phase 3) and floor driving arrive — they're explained in full below so you understand *why* they're coming, then built when they actually pay off.
...
Plugging a LiPo in backwards is the most common *fatal* mistake in this hobby — it instantly destroys drivers and regulators. ⚠️ **Your battery has bare ferruled leads, so nothing keys the polarity** — a keyed connector would normally make a backwards hookup physically impossible, but loose leads into a WAGO can absolutely go in wrong. In Phase 1 the safeguard is therefore *procedural, not a part*: **mark each WAGO + and −, and meter the leads (red = +) before every power-up.** Done consistently that's safe on a bench — but it's exactly the kind of human-error-prone step that a hardware reverse-protection part exists to eliminate, which is why you'll add one as the robot matures (or sooner, if you'd rather not rely on remembering to meter).

The naive hardware fix is a **series diode**: it only conducts one way, so a backwards battery does nothing. But a diode drops ~0.7 V *all the time*, even when correct, and at several amps that's several watts burned as heat for no benefit. The better fix is a **P-channel MOSFET high-side switch** (e.g. the Pololu #2815). A P-FET wired as an ideal-diode/reverse-protector has milliohms of resistance when conducting the right way — almost no voltage drop, almost no heat — yet blocks a reversed battery completely. Bonus: the #2815 is *also* a master ON/OFF switch, so one part does two safety jobs. We default to adding it in **Phase 3** (when the Pi arrives and lead-swapping gets routine), but with bare leads there's a fair argument to bring it in sooner — the "P-FET beats a diode" reasoning is a classic power-electronics lesson worth being able to explain.
...
In Phase 1, though, **the wheels are in the air on a stand** — there's no runaway robot to chase down — and you already have three instant kills: pull the **+** lead from its WAGO, type `stop` in the serial loop, or drop STBY (GP14) low. So the physical e-stop is **deferred to the first mobile phase**, where it actually earns its place; the design reasoning lives here so it's ready when you need it.
...
- [ ] **Dry continuity ritual (battery leads out of the WAGOs).** Multimeter on beep. Confirm a continuous ground from the star point to: both TB6612 GND pins, the Pico GND, and the (−) WAGO. Confirm **no** beep between any V+ (VM, 3.3 V) and any ground — that would be a short. Also check each WAGO: with one wire wiggled, all three ports of that connector should beep continuous to each other (a WAGO joins *all* inserted wires — there is no "in" vs "out").
...
- [ ] **Build and arm the power path (battery leads still out).** Assemble it dry, leaving the two battery leads *out* of the WAGOs for now: WAGO #1 (+) → both TB6612 VM; WAGO #2 (−) → both TB6612 GND. Land every power wire in its screw terminal and tug-test it. **Mark the WAGOs +/−. Meter the battery's leads (red = +) and confirm which is which** — this is the no-keying safeguard, do it every time. Only then insert the **+** lead into WAGO #1 and the **−** lead into WAGO #2 and close the levers. Measure **VM = 7.4 V** (up to ~8.4 V if freshly charged) on *both* TB6612 VM pins. Anything wildly off → **pull the + lead from its WAGO immediately** and recheck (suspect a swapped lead first).
...
- [ ] **Instant-kill test (the bench stand-ins for an e-stop).** With wheels spinning, verify each of your three kills works: (1) type `stop` in the serial loop → all four halt; (2) `enable_drivers(False)` (drops STBY/GP14 low) → all four go dead; (3) **lift the (+) WAGO lever and pull the + lead** → motors lose power while the Pico, still on laptop USB, stays alive (its LED stays on). The physical mushroom e-stop joins this list in the first mobile phase.
...
- **Battery on before logic.** Wrong order. Bring up logic first (Pico on laptop USB), *then* seat the battery leads in the WAGOs to power the motor rail. Logic must settle before motor power so the drivers don't glitch their outputs on a cold start.
- **⚠️ Reversed polarity (the new #1 fatal risk — your leads aren't keyed).** With bare ferruled leads, nothing stops you landing **+** in the **−** WAGO. That reverses VM on both TB6612s and destroys them instantly. **Every power-up:** meter the leads (red = +), confirm the WAGOs are marked +/−, insert + into the (+) WAGO and − into the (−) WAGO. If you ever measure a *negative* VM, pull power immediately — you've swapped them. (A keyed connector or the #2815 P-FET removes this risk; until then, the meter is your keying.)
- **Thinking a WAGO has an "in" and an "out."** It doesn't — all three ports of a 221-413 are electrically joined. "In vs out" is just *which wire happens to come from the battery*; the other two are both outputs to the two drivers. The battery's pre-crimped ferrules drop straight in: lift the lever, insert fully (the ferrule should bottom out, no bare conductor showing past the housing), close the lever, and tug-test. A half-seated lead is the usual cause of a driver that browns out under load. (If a ferrule is much longer than ~11 mm or has a bulky collar that won't seat, snip it and insert the bare strands — a 221 clamps bare fine-stranded wire directly.)
- **Skipping the 0.1 µF / bulk caps.** It usually "works" on the bench, then later the motor brush noise crashes the USB link mid-drive or sags the rail on stall. Solder the 0.1 µF at each motor body and the 470–1000 µF across VM *now*.
...
**2. Confirm the motor rail (multimeter, DC volts).** With the battery leads seated in the WAGOs, measure across each TB6612 **VM ↔ GND**. **Expected: 7.4 V** nominal (up to ~8.4 V freshly charged, sagging toward 6.0 V as the pack empties). If you see 0 V, suspect a half-seated lead in a WAGO; if both boards read 0 V together, the LiPo's protection board may have latched off (let it rest / recharge). **If you read a *negative* 7.4 V, your leads are reversed — pull power now.**
...
**What you learned.** You now understand, concretely, how a microcontroller turns a number into motion: an H-bridge reverses current direction, IN1/IN2 pick that direction while a 20 kHz PWM sets average voltage (and stays above hearing), and a STBY pin gates the whole driver. You also stood up the *muscle half* of the power path to actual current numbers — a protected LiPo on its ferruled flying leads, split through lever-nut connectors into two parallel driver rails landing on soldered screw terminals, with a bulk cap to ride out stall transients and a single-point star ground that keeps amps of motor-return current out of your logic reference. You also learned the unglamorous habit that bare, unkeyed leads force on you: **meter polarity before every power-up**, because nothing else will stop a reversed battery from killing both drivers. Just as important, you learned to *right-size protection to the situation*: why a single-branch bench build with a self-protected battery and wheels-in-the-air doesn't yet need the coordinated fuses, reverse-polarity P-FET, and fail-safe e-stop that a multi-branch, floor-driving robot does — and exactly when each of those gets added. And you learned the discipline that runs through the whole project: scope the PWM, measure the rail, and never trust a number you haven't seen on an instrument.
