...
| 2S LiPo 7.4 V 2200 mAh 10C (w/ built-in protection board) | 1 | **Phases 1–2 battery** (motors-only bench, ~2.6 A < the SM-2P's ~3 A). Protection board cuts on overcharge / over-discharge / overcurrent / short. Output = keyed **SM-2P male** plug; charge via a separate **DC5.5×2.5 barrel** jack with the 8.4 V charger. Don't cut the factory plug; use the SM-2P pigtail below. **Replaced from Phase 3 by the URGENEX pack** (full system ~10 A > SM-2P) — keep this as a spare. |
...
| **SM-2P female pigtail** (mates the kit LiPo's factory output plug) | 1 | $4 | **Phases 1–2 only.** Breaks the kit battery's keyed SM-2P male output out to red (+) / black (−) leads. Keyed → can't mate reversed; **don't cut the factory plug.** | https://www.amazon.com/Connector-VANDESAIL-Female-Adapter-Electrical/dp/B076HLQ4FX | 1–2 |
| **2S 7.4 V LiPo + USB charger kit** (URGENEX 2× 1800 mAh 35C, Deans T-plug) | 1 kit | $27 | **The robot's battery from Phase 3 on**, replacing the kit SM-2P pack. Full system draws ~5.4 A cruise / ~10 A worst (Pi-buck + motors + servos), over the SM-2P's ~3 A; the **Deans T-plug (~60 A)** clears it (35C × 1.8 Ah ≈ 63 A capable). **Includes a simple USB charger** (balances to 8.4 V — no balance-charger setup). Two packs → ~40 min total. ⚠️ Bare pack, **no PCB** → protection via the 10 A main fuse + #2815 + 2S alarm / Phase-4 INA219. | https://urgenexrc.com/products/urgenex-7-4v-rechargeable-lipo-rc-battery-2s-35c-1800mah-with-deans-t-plug-campatibal-with-most-1-10-1-16-scale-rc-cars-trucks-and-7-4v-1to2-usb-lipo-battery-charger | 3 |
| **Deans T-plug female pigtail** (battery → WAGO) | 1 | $3 | Lands the URGENEX pack's keyed T-plug onto red (+) / black (−) leads into the WAGO split — the T-plug analog of the SM-2P pigtail. Polarized (can't reverse-mate). | https://www.amazon.com/s?k=deans+t+plug+female+pigtail+lead | 3 |
...
| **M2.5 nylon standoff kit** | 1 | $10 | Pi 5 / STL-19P / breakouts are M2.5 (not M3); nylon = non-conductive + keeps ferrous mass off the magnetometer. **Board mounting begins in Phase 3 (the Pi) — not used in the Phase 1–2 bench build** (per-phase slice TBD in the mounting pass). | https://www.cytron.io/p-nylon-m2p5-pcb-standoff-kit-for-raspberry-pi-200pcs | 3 |
...
| Power & protection | ~$131 (+ Ph3 URGENEX 2S battery & USB-charger kit $27 + Deans T-plug pigtail $3; SM-2P pigtail + WAGO splices + two #2495 6-pin screw-terminal blocks for Ph1–2) |
