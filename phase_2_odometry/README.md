...
- **Phase 1 complete.** All four motors spin under Pico software control via the two TB6612FNG drivers; STBY is tied HIGH on GP14; the simplified bench motor path (LiPo **ferruled leads** → **WAGO** splits → both drivers' VM/GND on screw terminals) is built and verified — and you've got the polarity-meter habit down, since the bare leads aren't keyed. You can command each wheel individually from the Thonny REPL. (The full protection tree — fuses, #2815 switch, e-stop — and the Pi-buck/servo rails don't exist yet; they arrive in Phase 3.)
...
After Phase 1 you had a powertrain: a 2S LiPo (with its own built-in protection board) feeding a 7.4 V motor rail through its **ferruled flying leads** → two **WAGO 221-413** lever-nut splices (one for +, one for −) into two **TB6612FNG** dual H-bridge drivers, each driving two of the four 310 Mecanum motors. (No external fuses, master switch, or e-stop yet — those join the design in Phase 3 when the Pi becomes a second branch. And because the leads are bare/unkeyed, you meter polarity before each power-up.) The Pico 2W, powered over USB, drove the six logic lines per board (IN1/IN2/PWM × 2 channels) plus a shared **STBY** held HIGH on **GP14**, all referenced to a single-point star ground at the LiPo negative. The motors' factory **JST-PH 2.0 mm 6-pin** connectors mated to pigtails; the motor-power pair (M+, M−) ran on 18 AWG to the driver outputs, and the four encoder conductors in that same connector hung unused.
...
- [ ] **Continuity ritual first.** Battery leads out of the WAGOs. Beep-test that each encoder VCC reads to the Pico **3V3(OUT)**, *never* to the 7.4 V motor rail. No beep between any V+ and any GND. (See `safety.md` continuity ritual.)
